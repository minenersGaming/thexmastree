...
const HandleTree = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `http://localhost:3000/tree/element?id=${encodeURIComponent(id)}`
        );
  ...
  async function addElement(type: number, name: string, message: string) {
    ...
    if (!id) return;
    const response = await fetch(
      `http://localhost:3000/tree/addElement?id=${encodeURIComponent(id)}`,
      {
        method: "POST",
        headers: {
  ...
  const pageElements = receivedData.elements.filter(
    (e) => e.page === (pageNumber <= 0 ? 1 : pageNumber)
  );
  ...
  if (pageNumber === undefined || pageNumber <= 0) {
            ...
            <span className="anim-intro-fadeIn">
              ...
              <div className="flex flex-col justify-center w-full bottom-0 h-[60vh] items-center z-[300]">
                <img
                  ...
                  src={`/src/assets/onTree/${ITEMNAME[itemNumber]}.svg`}
                ></img>
                <p
    ...
    const maxPage =
      receivedData.elements.length > 0
        ? Math.max(...receivedData.elements.map((e) => e.page))
        : 1; //get max of page
              ...
              <div className="common-bg absolute top-0 left-0 w-full h-full">
                ...
                <div className="w-auto h-full left-0 flex flex-col justify-center z-[50] absolute px-[20px] items-center">
                  ...
                  >
                    <img
                      draggable="false"
                      src="/src/assets/icon/whiteArrow.svg"
                    ></img>
                  </button>
                </div>
